...
export const GetTask = () => {
  return async function (dispatch) {
    try {
      ...
      const res = await Promise.all([ContentServies.GetQuery(),ContentServies.GetStorage()]);

      const urls = await Promise.all(  res[1].items.map(async (item)=>{return {'url':await getDownloadURL(item),'name':item._location.path_}}));
